  ...
  return await Comment.sequelize!.transaction(async (transaction: Transaction) => {
    try {
      ...
      if (commentData.entityType === 'tour') {
        entityExists = await Tour.findByPk(commentData.entityId, { transaction }) !== null;
      } else if (commentData.entityType === 'lodging') {
        entityExists = await Lodging.findByPk(commentData.entityId, { transaction }) !== null;
      }
 ...
 */
const updateEntityRating = async (entityId: number, entityType: 'tour' | 'lodging', transaction: Transaction) => {
  try {
    ...
    if (count > 0) {
      ...
      if (entityType === 'lodging') {
        await Lodging.update({ rating: newRating }, { where: { id: entityId }, transaction });
